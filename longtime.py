...
if __name__ == "__main__":
    ...
    lower_bound = int(np.ceil((8/3) * t) - 1)
    upper_bound = int(np.ceil((8/3) * t) - 1)

    for m in range(1, 8 * t):

        graph_pebbling = GraphPebbling(m, m)

        if graph_pebbling.check_every_distribution(lower_bound, upper_bound, t) > np.ceil((8/3) * t) - 1:
            print(f"t = {t}\nm = {m}")
            break